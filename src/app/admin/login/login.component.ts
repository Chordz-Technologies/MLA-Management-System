...
export class LoginComponent {
  ...
  loginpost_data() {
    this.Admin_model.a_username = this.loginform.value.adminname,
      this.Admin_model.a_password = this.loginform.value.adminpassword;
    const selectedOption = this.loginform.value.options;
    this.Admin_model.a_typesupersuperadmin = selectedOption === '3' ? 1 : 0;
    this.Admin_model.a_typesuperadmin = selectedOption === '2' ? 1 : 0;
    this.Admin_model.a_typeadmin = selectedOption === '1' ? 1 : 0;
    // this.ElectionAdmin_model.utypeuser = selectedOption === '3' ? 1 : 0;

    this.logindata.loginpost(this.Admin_model).subscribe({
      next: (result: any) => {
        // if (result.message === 'Valid User' && result.admin_type === 'SuperSuperadmin') {
        //   const adminId = result.admin_ID;
        //   // const status = result.status;
        //   this.router.navigate(['/admin']);
        //   this.toastr.success('Login successful as a SuperSuperadmin!', 'Success');
        //   const admin_type: string = 'SuperSuperadmin';
